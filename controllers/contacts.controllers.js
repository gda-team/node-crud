...
exports.postNewContact = async(req, res, next) => {
	try {
		...
		await db.query('INSERT INTO contact (nom, prenom, email, tel, description) VALUES ($1, $2, $3, $4,$5)', [nom, prenom, email, tel, description]).then(data => {
...
exports.getOneContact = async(req, res, next) => {
	...
	const id = parseInt(req.params.id)
    await db.query('SELECT * FROM contact WHERE id = $1', [id]).then(data => {
		const onecontact = data.rows
		res.render('oneContact', {onecontact})
	})
	} catch (error) {
		console.log(error);
	}
...
exports.putUpdateContact = async(req, res, next) => {
	try {
		...
		await db.query('UPDATE contatc SET nom= $1, prenom= $2, email= $3, tel= $4, description=$5 WHERE id=$6', [nom, prenom, email, tel, description]).then(data => {
